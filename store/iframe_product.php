<?php 
    //跟manage_product差異
    //多<input class='relate_news' type='button' value='加入最新'..按鈕
    //多<input class='relate_news' type='button' value='加入熱門'..按鈕
    session_start();
        ...
        if(isset($_GET['keyword']))
        {
            switch($_GET['type'])
            {
                case '0':
                    ...
                    $query = "select * from goods_index where store=".$login_store." and `delete`=0 and fi_no in(".$_GET['keyword'].") ".$orderby;
                    break;
                case '1'://商品ID
                    $orderby = isset($_GET["oname"])?"order by ".$_GET["oname"]." ".$_GET["order"]:"order by fi_no asc";
                    $query = "select * from goods_index where store=".$login_store." and `delete`=0 and fi_no >= ".$_GET['keyword']." ".$orderby;
                    break;
                case '2'://商品關鍵字
                    $orderby = isset($_GET["oname"])?"order by ".$_GET["oname"]." ".$_GET["order"]:"order by category asc, fi_no asc";
                ...
                case '5'://供應商
                    $orderby = isset($_GET["oname"])?"order by ".$_GET["oname"]." ".$_GET["order"]:"order by fi_no asc";
                    $query = "select * from goods_index where store=".$login_store." and `delete`=0 and supplier like '%".$_GET['keyword']."%' ".$orderby;
                    break;
                case '6'://所有
                    break;
            }
            ...
            <div id="body">
                <?php
                    ...
                    for($i = 0; $i < $len; $i++)
                    {
                        ...
                        echo "<td style='width:80px;'><input class='relate_news' type='button' value='加入最新' style='height:30px;border:1px solid gray;background:white;'><br/><br/><input class='relate_hots' type='button' value='加入熱門' style='height:30px;border:1px solid gray;background:white;'></td>";
                        echo "</tr>";
                    }
                    echo "</table>";
                    echo "</div>";
