...
switch($_POST["query_type"])
{
    case "save_application_status":
        ...
        $query = "select progress from order_application where `order`=".$_POST["order"]." and member=".$_POST["member"]." and status=3";//status=3
        $data = $dba->query($query);
        ...
        {
            $query = "update order_index set application_rework=2 where fi_no=".$_POST["order"]." and member=".$_POST["member"];//application_rework
            $dba->query($query);
            $query = "update order_form set application_rework=2 where fi_no=".$_POST["order"]." and member=".$_POST["member"];
            $dba->query($query);
        }
        else
        {
            $query = "update order_index set application_rework=1 where fi_no=".$_POST["order"]." and member=".$_POST["member"];
            $dba->query($query);
            $query = "update order_form set application_rework=1 where fi_no=".$_POST["order"]." and member=".$_POST["member"];
            $dba->query($query);
        }
        break;
    case "get_order_apply":
        $query = "select * from order_application where `order`=".$_POST["fi_no"]." and member=".$_POST["member"]." and status=3";
        $data = $dba->query($query);
        $len = count($data);
        $goodsName = array();
        for($i=0;$i<$len;$i++)
