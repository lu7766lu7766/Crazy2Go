...
/** single bytes segment flag (純單字節字符) */
...
/** constant var define */
define ('PSCWS4_WORD_FULL',		0x01);	// 多字: 整詞
define ('PSCWS4_WORD_PART',		0x02);	// 多字: 前詞段
define ('PSCWS4_WORD_USED',		0x04);	// 多字: 已使用
define ('PSCWS4_WORD_RULE',		0x08);	// 多字: 自動識別的

define ('PSCWS4_ZFLAG_PUT',		0x02);	// 單字: 已使用
define ('PSCWS4_ZFLAG_N2',		0x04);	// 單字: 雙字名詞頭
define ('PSCWS4_ZFLAG_NR2',		0x08);	// 單字: 詞頭且為雙字人名
define ('PSCWS4_ZFLAG_WHEAD',	0x10);	// 單字: 詞頭
define ('PSCWS4_ZFLAG_WPART',	0x20);	// 單字: 詞尾或詞中
define ('PSCWS4_ZFLAG_ENGLISH',	0x40);	// 單字: 夾在中間的英文
define ('PSCWS4_ZFLAG_SYMBOL',	0x80);	// 單字: 符號系列
...
{	
	...
	function _ssegment($end)
	{
		...
		// 取出單詞及標點. 數字允許一個點且下一個為數字,不連續的. 字母允許一個不連續的'
		while ($start < $end)
		{
			$ch = $txt[$start++];
			$cx = ord($ch);
	...
	function _mset_word($i, $j)
	{
		...
		// hightman.070705: 加入 item == null 判斷, 防止超長詞(255字以上)unsigned char溢出
		...
		if (count($this->_rd) > 0)
		{
			...
			for ($i = 0; $i < $zlen; $i++)
			{
				...
				if (($r1['flag'] & PSCWS4_ZRULE_PREFIX) && ($i < ($zlen - $clen)))
				{
					...
					// 先檢查 zmin 字內是否全部符合要求, 再在 zmax 範圍內取得符合要求的字
					for ($ch = 1; $ch <= $clen; $ch++)
					{
						$j = $i + $ch;
			...
			// check for 'two words' (such as: 歐陽** , **西路)
			for ($i = $zlen - 2; $i >= 0; $i--)
			{
