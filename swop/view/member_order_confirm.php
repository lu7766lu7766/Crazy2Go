    ...
    <div style="margin-top: 15px;">
        ...
        <div style="display:inline-block;width:986px;float:right;margin-bottom:30px;">
            <div>
                ...
                <div style="display:inline-block;padding:10px 15px 10px 15px;margin:1px 0px;font-weight: bold;background:#FFFFFF;color:#000000;border-left:2px solid #D92F19;border-top:2px solid #D92F19;border-right:2px solid #D92F19;position:relative;top:3px;">已收貨待確認訂單</div>
                    ...
                    for($i = 0;$i<$len;$i++)
                    {
                        ...
                        for($j=0; $j<$len2; $j++)
                        {
                            ...
                            $returnBtn = "<input class='returns_button' type='button' value='$btnValue' ".
                                         ...
                                         "images='".$this->base["thu"].$this->order[$i]["goods"][$j]["images"]."' ".
                            ...
                            if($j == 0)
                            {
                                echo    '<div class="td" style="border-left:1px solid #D4D4D4;padding:10px;width:115px;"><span style="color:#E93439;font-weight:bold;"><span style="font-family:Arial;">¥</span>'.$this->order[$i]["payments"].'</span><br/><span style="color:#838383;">(含運費<span style="font-family:Arial;">¥</span>'.$this->order[$i]["shipping_fee"].')</span></div>'.
                                        '<div class="td" style="border-left:1px solid #D4D4D4;padding:10px;width:103px;"><input class="icheckbox order_success" name="order_success" type="checkbox"><label for="order_success" style="line-height:12px;" >賣家已出貨</label> <br/><input class="transaction_complete" type="button" value="交易完成確認" order="'.$this->order[$i]["goods"][$j]["order"].'"><br/><input class="check_logistics" type="button" value="查看物流" seller="'.$this->order[$i]["name"].'" sn="'.$this->order[$i]["sn"].'" order="'.$this->order[$i]["goods"][$j]["order"].'"><br/><input class="rank_seller" type="button" value="評論賣家" seller="'.$this->order[$i]["name"].'" order="'.$this->order[$i]["goods"][$j]["order"].'" sn="'.$this->order[$i]["sn"].'"></div>';
                            }
                            else
                            {
