<?php 
    ...
    if(!empty($_SESSION['admin']['login_user'])){
        ...
        $pager = new Pager();
        $all_data = $pager->query("select `fi_no`,`category`,`name`,`logo`,`weights` from `brand_group` where `delete`='0'  order by `weights` desc");
        $dba = $pager->get_dba();
    ...
    <head>
        ...
        <script>
            ...
            $(document).ready(function(){
                ...
                function init(){
	                ...
	                if($("#addPanel").length>0){
		                ...
		                $("input.add_btn").click(function(){
		                    ...
		                    $this.find("select[name*='add_category']").each(function(){
			                    a_category.push($(this).val());
		                    })
		                    var json_category = JSON.stringify(a_category);
		                    //console.log(json_category+"^^"+name+"^^"+filename+"^^"+"<?php echo $subject;?>_add")
		                    //return;
	                ...
	                $("#list_panel [data-open-dialog]").mouseenter(function(){
						...
						$dialog.find("select.category_sel").remove();
						$dialog.find("select:hidden").each(function(index){
							$.post(filename,{
									...
									$dialog.find("select:hidden").eq(index).after($sel);
									update_select("add");
									//console.log($dialog.find("select:hidden").eq(index))
		                    });
		                    $(this).unbind("change");
		                    $(this).change(function(){
			                    $data = $("#data_"+edit_no);
			                    $data.attr($(this).attr("id"),$(this).val());
		                    })
						})
	                });
	                ...
	                function get_data(now_no,$panel){
		                ...
		                var $data   = $("#"+data_id);
		                ///////////////////////////////edit_category_length
		                var $td = $panel.find("select[name*=edit_category]").parent();
		                $select = $panel.find("select[name*=edit_category]:first").clone();
		                
		                $panel.find("select[name*=edit_category]").remove();
		                $td.find("br").remove();
		                for(i=0;$data.attr("edit_category"+i)!=undefined;i++)
		                {
			                $selected = $select.clone();
			                sel_name = $selected.attr("name").replace(/\d+/g,"");
			                $selected.attr("name",sel_name+i);
		        			$selected.attr("id",sel_name+i);
		        			$selected.find("option[selected]").removeAttr("selected");
							$selected.find("option[value="+$data.attr(sel_name+i)+"]").attr("selected","selected");
		                
			                $td.append($selected)
			                if($data.attr(sel_name+(i+1))!=undefined)
			                {
				                $td.append("<br>")
			                }	
		                }
	                ...
	                $panel.find("input.edit_btn").click(function(){
	                    ...
	                    for(i=0;$data.attr("edit_category"+i)!=undefined;i++)
	                    {
		                    a_category.push($data.attr("edit_category"+i));
	                    }
	                    var json_category = JSON.stringify(a_category);
						...
						form_data.append("category",	json_category);
				...
				$(".add_category_select").click(function(){
					$td = $(this).parent().next();
        			$selected = $td.find("select:hidden:first").clone();
        			len = $td.find("select:hidden").length
        			sel_name = $selected.attr("name").replace(/\d+/g,"");
        			$selected.attr("name",sel_name+len);
        			$selected.attr("id",sel_name+len);
        			$selected.find("option[selected]").removeAttr("selected");
        			$selected.find("option:first").attr("selected","selected");
        			//$selected.find("option:first").val( $selected.find("option:first").attr("value") );
        			$td.append("<br>").append($selected);
        			$td.find("select.category_sel").remove();
        			$td.find("select:hidden").each(function(index){
						$.post(filename,{
	                    		 query_type:"get_category_back"
	                    		,fi_no:$td.find("select:hidden").eq(index).val()
							},function(data){
								
								$td.find("select:hidden").eq(index).after(data);
								update_select("add");
	                    });
	                    if(edit_no!=undefined)
	                    {
		                    $(this).unbind("change");
		                    $(this).change(function(){
			                    $data = $("#data_"+edit_no);
        ...
        <div id="wrapper">
            ...
            <div id="body">
                ...
                <div id="body_right">
                	...
                	<?php
                        if($permissions2add){
                        	echo "<input type='button' value='新增{$subject_cht}' data-open-dialog='新增{$subject_cht}' onclick='javascript:edit_no=undefined'>";
							...
							echo "	<tr>
                            			<td>
                            				分類<br>
                            				<input type='button' class='add_category_select' value='增加分類' />
										</td>
                            			<td>
                            				<select name='add_category0' id='add_category0' style='display:none;'/>
						    					{$category_html}
											</select>
                            			</td>
									</tr>";
					...
					<table class='table-h' id='list_panel'>
						...
						foreach($all_data as $per_data){
							...
							//$tmp_category	= '["3","4","5","6","8"]';
							$a_category		= json_decode($tmp_category,true); 
						...
						<tr>
                            ...
                            foreach($a_category as $key=>$category)
                            {
	                            echo " 	edit_category".$key."='$category' ";
                            }
                            echo "		edit_weights='$tmp_weights' />";
                            if($tmp_logo!="")
