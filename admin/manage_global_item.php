<?php 
    ...
    if(!empty($_SESSION['admin']['login_user'])){
        ...
        $sel = $_GET["sel"];
        $sel = $sel==""?0:$sel;
        $type_data = $dba->query("select distinct `type` from `global_item`");
        
        foreach($type_data as $per_data)
        {
	    	$type_data = $per_data["type"];
	    	switch($type_data){
		    	case "1":
		    		$type_data_cht = "頁首連結";
		    	break;
		    	...
		    	break;
		    	case "4":
		    		$type_data_cht = "頁尾次要連結";
		    	break;
		    	default:
		    		$type_data_cht = $type_data;
		    	break;
	    	}
	        $show_list_html.="<option value='$type_data' ".($sel==$type_data?"selected":"").">$type_data_cht</option>";
	    }
        ...
        <script>
            ...
            {	
                ...
                {	
	                ...
	                $panel.find("input.edit_btn").click(function(){
						...
						$obj.find("input:text").each(function(index){
							form_data.append( 'items['+index+']',$(this).val() );
						})
						$obj = $( ".tr_urls" );//text
						$obj.find("input:text").each(function(index){
							form_data.append( 'urls['+index+']',$(this).val() );
						})
						...
						form_data.append("json_item",	item);
				...
				$("input.preview_adpos").click(function()
				{
					...
					{
						var page = $("input[name='add_type']").val();
					}
					...
					if( page=='1')
					{
						...
						iframe_target_class = ".quick_top";
					}
					else if( page=='2')
					{
						iframe_link = "http://www.crazy2go.com/"+iframe_parameter;
						iframe_target_class = ".quick_bottom";
					}
					else if( page=='3')
					{
						iframe_link = "http://www.crazy2go.com/"+iframe_parameter;
						iframe_target_class = ".quick_sidebar";
					}
					else if( page=='4')
					{
						iframe_link = "http://www.crazy2go.com/"+iframe_parameter;
						iframe_target_class = ".quick_bottom_second";
					}
	...
	<script>
		$(document).ready(function(){
	    	$("select[name='show_list_panel']").change(function(){
		    	...
		    	$("#add_type").find("option[selected]").removeAttr("selected");
		    	$("#add_type").find("option[value="+$(this).val()+"]").attr("selected","selected");
	        	...
	        	pic_size.push(Array(20,20));
	        	pic_size.push(Array(25,25));
	        	pic_size.push(Array(0,0));
	    	...
	    	$("select[name='show_list_panel']")
                            ...
                            <tr class='tr_icon' style='display:none;'>
                            	<td width="150px">
	                            	圖片<br><span class='pic_size'></span>
	                            </td>
                                <td>
                                	<input type='file' class='pic' name='edit_icon' max_len='1' multiple='multiple'/>
                                </td>
                            <tr class='tr_url'>
                            	<td>URL</td>
                                <td>
                            ...
                            <tr class='tr_items' style='display:none;'>
                            	<td>Items</td>
                                <td  class='edit_items' copy='edit_items'>
                                </td>
                            </tr>
                            <tr class='tr_urls' style='display:none;'>
                            	<td>URLs</td>
                                <td class='edit_urls' copy='edit_urls'>
                                </td>
                            </tr>
                            <tr>
                            	<td>是否顯示</td>
