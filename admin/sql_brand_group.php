...
switch($_POST["query_type"]){
	...
	case $subject."_add":
		...
		$result = $dba->query("select max(weights) from `brand_group`;");
		$weights = $result[0]["max(weights)"]+1;
		
		$sql =	"insert into `brand_group` (`category`,`name`,`weights`) values('$category','".${$subject."_name"}."','$weights');";
	...
	case $subject."_del":
		...
		$sql = "delete from `brand_group` where `fi_no`='".${$subject."_no"}."'";
		$result	= $dba->query($sql);
        if($result){
        	$dba->query("insert into administrator_log (`administrator`,`action`,`action_date`) 
